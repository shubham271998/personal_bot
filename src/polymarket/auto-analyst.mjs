...
const VIRTUAL_MAX_BET = 40 // $40 max (sports $10 cap in Smart Brain)
const VIRTUAL_MAX_OPEN = 80 // 80 open positions — cover more markets simultaneously
...
async function runVirtualTradingCycle() {
  ...
  try {
    ...
    for (const pick of brainScan.approved.slice(0, 10)) {
      if (openPositions.length + tradesPlaced >= VIRTUAL_MAX_OPEN) break
      if (!pick.market?.id) continue
